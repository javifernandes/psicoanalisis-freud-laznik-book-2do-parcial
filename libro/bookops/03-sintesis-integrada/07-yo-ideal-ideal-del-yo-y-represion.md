...
```mermaid
flowchart TD
  A["Ideal del Yo"] -->|"Mide"| B["Yo actual"]
  C["Yo ideal"] -->|"Ofrece imagen de completitud"| A
  D["Identificaciones parentales y sociales"] --> A
  B --> E{"Distancia respecto del Ideal"}
  E --> F["Satisfacción admitida"]
  E --> G["Displacer e inconciliabilidad"]
  G --> H["Represión"]
```
...
```mermaid
flowchart LR
  A["Identificación primordial"] --> C["Base del Ideal del Yo"]
  B["Identificaciones secundarias"] --> C
  C --> D["Dirección de investiduras"]
  C --> E["Medida del Yo actual"]
  E --> F["Condición del conflicto represivo"]
```
...
```mermaid
flowchart TD
  subgraph T["Lectura de Tótem y tabú"]
    A["Padre muerto"] --> B["Obediencia retrospectiva"]
    B --> C["Ley y prohibición"]
  end
  subgraph N["Lectura del narcisismo"]
    D["Identificación primordial"] --> E["Ideal del Yo"]
    E --> F["Medida y represión"]
  end
  A -.->|"Paralelismo de la cátedra"| D
  C -.->|"Función ordenadora"| E
```
...
```mermaid
flowchart LR
  A["Yo actual"] --> B["Ideal no alcanzado"]
  B --> C["Proyección sobre el objeto"]
  C --> D["Idealización"]
  D --> E["Engrandecimiento del objeto"]
  D --> F["Empobrecimiento del Yo"]
```
