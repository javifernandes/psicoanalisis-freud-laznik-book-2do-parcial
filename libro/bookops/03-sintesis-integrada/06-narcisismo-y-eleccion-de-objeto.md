...
flowchart LR
  ...
  A --> D["Pulsiones parciales"]
  B --> E["Yo como objeto unificado"]
  C --> F["Objeto unificado de amor"]
  D -.->|"Rasgos parciales persisten"| F
```
...
```mermaid
flowchart LR
  A["Yo"] -->|"Investidura"| B["Objeto"]
  B -->|"Retirada"| A
  A --> C["Libido yoica"]
  B --> D["Libido de objeto"]
```
...
```mermaid
flowchart TD
  A["Libido concentrada en el Yo"] --> B["Estasis libidinal"]
  B --> C["Incremento de tensión y displacer"]
  C --> D["Investidura de objetos"]
  D --> E["Nueva distribución económica"]
```
...
```mermaid
flowchart TD
  A["Marcas tempranas de satisfacción"] --> D["Elección actual"]
  B["Figuras de cuidado"] --> D
  C["Imagen e Ideal del Yo"] --> D
  E["Rasgos pulsionales parciales"] --> D
  D --> F["Objeto de amor"]
```
