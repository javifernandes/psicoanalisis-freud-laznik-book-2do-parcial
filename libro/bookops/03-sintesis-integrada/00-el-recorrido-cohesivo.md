...
```mermaid
flowchart TD
  A["Caída del adulto seductor como causa universal"] --> B["Sexualidad infantil"]
  B --> C["Pulsión"]
  C --> D["Exigencia de trabajo para lo psíquico"]
  D --> E["Fantasía y teorías sexuales infantiles"]
  D --> F["Inscripción pulsional"]
  C --> G["Autoerotismo"]
  G --> H["Identificación y constitución del Yo"]
  H --> I["Narcisismo e Ideal del Yo"]
  I --> J["Condiciones del conflicto"]
  F --> K["Representante psíquico de la pulsión"]
  K --> L["Represión"]
  J --> L
  L --> M["Sistema inconsciente"]
  L --> N["Retorno de lo reprimido"]
  E --> O["Síntoma"]
  N --> O
  O --> P["Satisfacción sustitutiva"]
  N --> Q["Repetición"]
  Q --> R["Transferencia"]
  P --> R
  R --> S["Recordar y reelaborar"]
  L --> T["Destino del afecto"]
  T --> U["Angustia"]
```
