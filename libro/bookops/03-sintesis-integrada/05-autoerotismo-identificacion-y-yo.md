...
```mermaid
flowchart TD
  A["Cuerpo no unificado"] --> B["Zona oral"]
  A --> C["Zona anal"]
  A --> D["Mirada"]
  A --> E["Crueldad o apoderamiento"]
  B --> F["Satisfacciones parciales"]
  C --> F
  D --> F
  E --> F
  F -.->|"Todavía no forman una unidad"| G["Yo"]
```
...
flowchart LR
  ...
  B --> C["Identificación"]
  C --> D["Constitución de una unidad yoica"]
  D --> E["Yo como objeto de la libido"]
  E --> F["Narcisismo"]
```
...
```mermaid
flowchart TD
  A["Campo autoerótico"] --> B["Nuevo acto e identificación"]
  B --> C["Yo unificado"]
  C --> D["Libido yoica"]
  C --> E["Libido de objeto"]
  A --> F["Resto autoerótico"]
  F -.->|"No se integra por completo"| C
  F --> G["Persistencia de satisfacciones parciales"]
```
