...
```mermaid
flowchart TD
  A["Moción pulsional"] --> B["Representación"]
  A --> C["Monto de afecto"]
  B --> D["Represión o sustitución"]
  C --> E["Sofocación"]
  C --> F["Afecto cualificado"]
  C --> G["Mudanza en angustia"]
```
...
```mermaid
flowchart TD
  A["Exigencia pulsional"] --> B["Representación inconciliable"]
  B --> C["Represión"]
  C --> D["Defensa del Yo"]
  C --> E["Retorno deformado"]
  D --> F["Síntoma"]
  E --> F
  A -->|"Satisfacción indirecta"| F
  F --> G["Padecimiento consciente"]
  F --> H["Satisfacción inconsciente"]
```
...
```mermaid
flowchart TD
  A["Moción libidinosa hacia el padre"] --> B["Conflicto y represión"]
  B --> C["Representación"]
  B --> D["Monto de afecto"]
  C --> E["Animal sustitutivo"]
  D --> F["Angustia"]
  E --> G["Miedo fóbico"]
  F --> G
  G --> H["Intento de huida exterior"]
```
...
```mermaid
flowchart LR
  A["Representación reprimida"] --> B["Condensación"]
  A --> C["Desplazamiento"]
  A --> D["Sustitución"]
  B --> E["Retoño deformado"]
  C --> E
  D --> E
  E --> F["Síntoma, sueño, lapsus o repetición"]
```
