...
```mermaid
flowchart TD
  A["1. Pulsión y fantasía<br/>¿Qué clase de sexualidad exige trabajo psíquico?"] --> B["2. Narcisismo y constitución del Yo<br/>¿Cómo se forman una unidad yoica y sus objetos?"]
  B --> C["3. Metapsicología<br/>¿Cómo se inscriben, reprimen y conservan eficaces las mociones?"]
  C --> D["4. Transferencia y técnica<br/>¿Cómo retorna el conflicto dentro de la cura?"]
  C --> E["5. Angustia<br/>¿Cómo afecta al Yo el factor cuantitativo?"]
  D --> E
```
...
flowchart LR
  ...
  B --> C["Exigencia de trabajo"]
  C --> D["Fantasías y teorías sexuales"]
  C --> E["Conflicto pulsional"]
  E --> F["Síntoma"]
```
...
```mermaid
flowchart LR
  A["Autoerotismo"] --> B["Nuevo acto psíquico"]
  B --> C["Yo como unidad"]
  C --> D["Narcisismo"]
  D --> E["Elección de objeto"]
  C --> F["Identificaciones"]
  F --> G["Ideal del Yo"]
  G --> H["Condición de la represión"]
```
...
```mermaid
flowchart LR
  A["Representante psíquico"] --> B["Represión primordial"]
  B --> C["Represión propiamente dicha"]
  C --> D["Sistema inconsciente"]
  D --> E["Retoños y retorno"]
  E --> F["Síntoma"]
```
...
```mermaid
flowchart LR
  A["Clichés infantiles"] --> B["Transferencia"]
  B --> C["Motor de la cura"]
  B --> D["Resistencia"]
  D --> E["Repetición en acto"]
  E --> F["Reelaboración"]
```
...
flowchart LR
  ...
  B --> C["Angustia"]
  C --> D["Síntoma"]
  D --> E["Ligadura, acotamiento y evitación"]
```
